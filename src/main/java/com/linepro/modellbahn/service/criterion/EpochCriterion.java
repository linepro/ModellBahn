...
public class EpochCriterion extends AbstractCriterion {
    ...
    @Override
    public Predicate[] getCriteria(CriteriaBuilder criteriaBuilder, CriteriaQuery<?> query, Root<?> epoch) {
        List<Predicate> where = new ArrayList<>();
        addCondition(criteriaBuilder, epoch, where, DBNames.NAME, getName());
        addCondition(criteriaBuilder, epoch, where, DBNames.BEZEICHNUNG, getBezeichnung());
        addCondition(criteriaBuilder, epoch, where, DBNames.DELETED, getDeleted());
